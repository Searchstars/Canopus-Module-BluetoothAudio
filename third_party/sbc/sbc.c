...
SBC_EXPORT size_t sbc_get_private_size(void)
{
	return sizeof(struct sbc_priv) + SBC_ALIGN_MASK;
}

SBC_EXPORT int sbc_init_static(sbc_t *sbc, void *storage, size_t storage_len,
				unsigned long flags)
{
	uintptr_t aligned;

	if (!sbc || !storage || storage_len < sbc_get_private_size())
		return -EINVAL;
	memset(sbc, 0, sizeof(*sbc));
	aligned = ((uintptr_t)storage + SBC_ALIGN_MASK) & ~((uintptr_t)SBC_ALIGN_MASK);
	sbc->priv = (void *)aligned;
	sbc->priv_alloc_base = storage;
